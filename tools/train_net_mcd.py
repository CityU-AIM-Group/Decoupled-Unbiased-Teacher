...
def train(cfg, local_rank, distributed):
    ...
    if stage != 1:
        ...
        # predictor_t_aux = build_rpn(cfg, generator_t.out_channels).to(device)
        # predictor_s_aux = build_rpn(cfg, generator_s.out_channels).to(device)
    ...
    if distributed:
        if stage != 1:
            ...
            predictor_t = torch.nn.parallel.DistributedDataParallel(predictor_t, device_ids=[local_rank], \
                output_device=local_rank, broadcast_buffers=False)
        # predictor_t_aux = torch.nn.parallel.DistributedDataParallel(predictor_t_aux, device_ids=[local_rank], \
            # output_device=local_rank, broadcast_buffers=False)
        ...
        predictor_s = torch.nn.parallel.DistributedDataParallel(predictor_s, device_ids=[local_rank], \
            output_device=local_rank, broadcast_buffers=False)
        # predictor_s_aux = torch.nn.parallel.DistributedDataParallel(predictor_s_aux, device_ids=[local_rank], \
            # output_device=local_rank, broadcast_buffers=False)
    ...
    # model["predictor_t_aux"] = predictor_t_aux
    model["predictor_s"] = predictor_s
    # model["predictor_s_aux"] = predictor_s_aux
