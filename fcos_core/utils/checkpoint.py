...
class DetectronCheckpointer(Checkpointer):
    ...
    def save(self, name, **kwargs):
        ...
        data = {}
        if "generator_t" in self.model.keys():
    ...
    def _load_model(self, checkpoint):
        ...
        elif checkpoint["model"].get("model_backbone"):
            self.logger.info("Loading the source only model v2 ==> ")
            load_state_dict(self.model["generator_t"], checkpoint["model"]["model_backbone"])
            load_state_dict(self.model["generator_s"], checkpoint["model"]["model_backbone"])
            load_state_dict(self.model["predictor_t"], checkpoint["model"]["model_fcos"])
            load_state_dict(self.model["predictor_s"], checkpoint["model"]["model_fcos"])
        else:
